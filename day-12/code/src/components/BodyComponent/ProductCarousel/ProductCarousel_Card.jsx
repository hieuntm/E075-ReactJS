...
const extractNumberFromPercentage = (str) => parseInt(str.match(/\d+/)[0]);

export function ProductCarousel_Card({ product }) {
  const { CPU: cpu, GPU: gpu, mainboard, ram, disk } = product.detail;

  const currentPrice =
    product.price -
    Math.floor(
      (extractNumberFromPercentage(product.currentSale) * product.price) / 100
    );
  ...
  return (
    <div className="product-carousel-card">
      ...
      <div className="product-carousel-card-content">
        <p className="product-carousel-card-content-title">{product.name}</p>
        ...
        <p className="product-carousel-card-content-detail-current-price">
          {formatCurrency(currentPrice)} <span>-{product.currentSale}</span>
        </p>
